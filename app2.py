...
tools = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Analyze a SQL query for complexity and performance issues, providing optimization recommendations"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "The SQL query to analyze for complexity and potential issues"
                    }
                },
                "required": ["query"]
            },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Monitor a SQL query's execution time and performance metrics, providing detailed analysis"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "The SQL query to analyze for performance"
                    }
                },
                "required": ["query"]
            },
...
def get_anthropic_llm():
    ...
    if not api_key:
        ...
        return None
    return Anthropic(model="claude-3-opus-20240229", api_key=api_key)
...
async def monitor_query_performance(query):
    """
    Monitor the execution time and performance metrics of a SQL query.
    
    Args:
        query (str): The SQL query to monitor
        
    Returns:
        str: Detailed performance metrics report
    """
    import time
    import json
    ...
    result_output = "Query Performance Analysis\n"
    result_output += "========================\n\n"
    result_output += f"Query: {query}\n\n"
    result_output += f"Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    
    try:
        # First, collect the EXPLAIN ANALYZE output
            ...
            async with ClientSession(*streams) as session:
                ...
                if hasattr(connect_result, 'content') and connect_result.content:
                    ...
                    if hasattr(content, 'text'):
                        ...
                        return "Error: Connection response missing text content"
                else:
                    return "Error: Connection response missing content"
                ...
                try:
                    ...
                    if hasattr(explain_result, 'content') and explain_result.content:
                        for item in explain_result.content:
                            if hasattr(item, 'text') and item.text:
                                try:
                                    data = json.loads(item.text)
                                    if isinstance(data, list) and len(data) > 0 and 'Plan' in data[0]:
                                        explain_data = data[0]
                                        break
                                except json.JSONDecodeError:
                                    pass
                    
                    # Now run the actual query to measure real-world execution
                    ...
                    # Format the results
                    result_output += "Timing Metrics:\n"
                    result_output += "---------------\n"
                    result_output += f"Connection time: {connect_time:.6f} seconds\n"
                    result_output += f"Explain analysis time: {explain_time:.6f} seconds\n"
                    result_output += f"Actual execution time: {actual_time:.6f} seconds\n"
                    result_output += f"Total time (including overhead): {total_time:.6f} seconds\n\n"
                    
                    # Add detailed execution plan metrics if available
                    if explain_data and 'Plan' in explain_data:
                        plan = explain_data['Plan']
                        
                        result_output += "Execution Plan Metrics:\n"
                        result_output += "----------------------\n"
                        result_output += f"Execution time (from EXPLAIN): {plan.get('Actual Total Time', 0):.6f} ms\n"
                        result_output += f"Planning time: {explain_data.get('Planning Time', 0):.6f} ms\n"
                        result_output += f"Execution memory: {plan.get('Peak Memory Usage', 0)} kB\n"
                        result_output += f"Rows returned: {plan.get('Actual Rows', 0)}\n"
                        result_output += f"Loops: {plan.get('Actual Loops', 0)}\n\n"
                        
                        # Extract node types from the plan
                        node_types = extract_node_types(plan)
                        result_output += "Query Plan Node Types:\n"
                        for node_type, count in node_types.items():
                            result_output += f"• {node_type}: {count}\n"
                        result_output += "\n"
                        
                        # Add performance insights
                        result_output += "Performance Insights:\n"
                        result_output += "--------------------\n"
                        insights = generate_performance_insights(plan, actual_time, node_types)
                        for insight in insights:
                            result_output += f"• {insight}\n"
                        
                except Exception as e:
                    result_output += f"Error executing query: {str(e)}\n"
                ...
                try:
                    ...
                    result_output += f"Warning - Error during disconnect: {str(e)}\n"
                
                return result_output
                
    except Exception as e:
        return f"Error monitoring query performance: {str(e)}"

def extract_node_types(plan):
    """Extract and count all node types in a query execution plan."""
    node_types = {}
...
def generate_performance_insights(plan, actual_time, node_types):
    """Generate performance insights based on the execution plan."""
    insights = []
    
    # Check for sequential scans
    if 'Seq Scan' in node_types:
        insights.append(f"Sequential scan detected ({node_types.get('Seq Scan')}) - Consider adding indexes to improve performance")
    
    # Check if the query is using indexes well
    if 'Index Scan' in node_types or 'Index Only Scan' in node_types:
        insights.append(f"Query is utilizing indexes ({node_types.get('Index Scan', 0) + node_types.get('Index Only Scan', 0)} scans)")
    
    # Check for large sorts
    if 'Sort' in node_types:
        if plan.get('Actual Rows', 0) > 10000:
            insights.append(f"Large sort operation on {plan.get('Actual Rows', 0)} rows - Consider adding indexes to avoid sorting")
    
    # Check for nested loops with many iterations
    if 'Nested Loop' in node_types and plan.get('Actual Loops', 0) > 1000:
        insights.append(f"Nested loop with {plan.get('Actual Loops', 0)} iterations - Consider restructuring the query")
    
    # Check execution time
    if actual_time < 0.1:
        insights.append("Query execution time is excellent (< 100ms)")
    elif actual_time < 1.0:
        insights.append("Query execution time is good (< 1s)")
    elif actual_time < 3.0:
        insights.append("Query execution time is acceptable (< 3s)")
    else:
        insights.append(f"Query execution time is slow ({actual_time:.2f}s) - Consider optimization")
    
    # If no specific insights were generated
    if len(insights) <= 1:  # Only has the time insight
        insights.append("No major performance issues detected in the execution plan")
...
async def analyze_query_complexity(query):
    """
    Analyze a SQL query for complexity and efficiency issues.
    
    Args:
        query (str): The SQL query to analyze
        
    Returns:
        str: Analysis of the query's complexity and suggestions for improvement
    """
    ...
    if "SELECT *" in query_upper:
        complexity_score += 2
        warnings.append("Query uses 'SELECT *' which retrieves unnecessary columns")
        recommendations.append("Specify only needed columns instead of using SELECT *")
    
    # Check for multiple joins
    join_count = (query_upper.count(" JOIN ") + 
                  query_upper.count(" INNER JOIN ") + 
                  query_upper.count(" LEFT JOIN ") + 
                  query_upper.count(" RIGHT JOIN ") + 
                  query_upper.count(" FULL JOIN ") +
                  query_upper.count(" CROSS JOIN "))
    
    if join_count > 0:
        analysis.append(f"Query contains {join_count} JOIN operations")
        if join_count > 3:
            complexity_score += join_count
            warnings.append(f"High number of JOINs ({join_count}) may impact performance")
            recommendations.append("Consider if all joins are necessary or if data structure could be optimized")
    
    # Check for aggregations
    aggregation_keywords = ["SUM(", "COUNT(", "AVG(", "MAX(", "MIN(", "GROUP BY", "HAVING"]
    aggregation_count = sum(query_upper.count(keyword) for keyword in aggregation_keywords)
    if aggregation_count > 0:
        analysis.append(f"Query uses {aggregation_count} aggregation operations")
        if aggregation_count > 3:
            complexity_score += 1
            analysis.append("Multiple aggregations increase computational complexity")
    
    # Check for subqueries
    subquery_count = query.count("(SELECT")
    if subquery_count > 0:
        complexity_score += subquery_count * 2
        warnings.append(f"Query contains {subquery_count} subqueries which may be optimizable")
        recommendations.append("Evaluate if subqueries could be replaced with JOINs for better performance")
    
    # Check for ORDER BY on large result sets
    if " ORDER BY " in query_upper and "LIMIT " not in query_upper:
        complexity_score += 2
        warnings.append("Query uses ORDER BY without LIMIT which may be inefficient for large datasets")
        recommendations.append("Add a LIMIT clause when using ORDER BY on large tables")
    
    # Check for lack of WHERE clause in SELECT statements
    if query_upper.startswith("SELECT") and " FROM " in query_upper and " WHERE " not in query_upper:
        # Exclude queries with GROUP BY as they might intentionally get all rows
        if " GROUP BY " not in query_upper:
            complexity_score += 1
            warnings.append("Query selects data without a WHERE clause, potentially retrieving unnecessary rows")
            recommendations.append("Add appropriate WHERE conditions to filter data")
    
    # Check for using DISTINCT
    if " DISTINCT " in query_upper:
        complexity_score += 1
        analysis.append("Query uses DISTINCT which can be expensive on large datasets")
        recommendations.append("Consider if DISTINCT is necessary or if it could be avoided with better JOIN conditions")
    
    # Check for LIKE with leading wildcard
    if "'%" in query or "\"%"in query:
        complexity_score += 2
        warnings.append("Query contains LIKE conditions with leading wildcards ('%...') which cannot use standard indexes")
        recommendations.append("Avoid leading wildcards in LIKE conditions when possible, or implement full-text search for this use case")
    
    # Check for complex expressions in WHERE clause
    complex_where_patterns = ["CASE WHEN", "COALESCE(", "NULLIF(", "CAST(", "CONVERT(", "::"]
    complex_where_count = sum(query_upper.count(pattern) for pattern in complex_where_patterns)
    if complex_where_count > 0:
        complexity_score += complex_where_count
        warnings.append(f"Query contains {complex_where_count} complex expressions in filtering conditions")
        recommendations.append("Move complex calculations to derived tables or CTEs when possible")
    ...
    result = f"SQL Query Complexity Analysis\n"
    result += f"===========================\n\n"
    result += f"Complexity Score: {complexity_score} ({complexity_rating})\n\n"
    ...
    if not warnings and not recommendations:
        result += "No obvious issues found. The query appears to be well-optimized.\n\n"
    
    result += "Note: This is a basic static analysis and doesn't account for database-specific optimizations, \n"
    result += "table sizes, or index configurations which would affect actual performance."
...
async def main():
    ...
    orchestration_agent = FunctionAgent(
        name="OrchestratorAgent",
        description="Routes user requests to the appropriate specialized agent based on the type of query",
        system_prompt="""
You are an intelligent query orchestrator that determines how to handle database queries efficiently.

DECISION RULES:
1. If the user provides a natural language query (like "show me all users who signed up last month"):
   - Route to the DB Explorer agent to convert to SQL and execute

2. If the user provides a SQL query (starts with SELECT, INSERT, UPDATE, etc.):
   - First route to the Query Analyzer to check for performance issues and get recommendations
   - If the Query Analyzer finds significant issues (complexity score > 10), suggest improvements before execution
   - If no major issues are found, route to DB Explorer to execute the query

3. If the user explicitly asks for query analysis or optimization:
   - Route only to the Query Analyzer, regardless of query format

4. Always run performance monitor in the end.

Always explain your routing decision briefly before calling the appropriate agent.
""",
        llm=llm,
        tools=[],
        can_handoff_to=["DB Explorer", "Query Analyzer", "Performance Monitor"],
    )

    performance_agent = FunctionAgent(
    name="Performance Monitor",
    description="An agent that monitors SQL query execution time and provides performance metrics",
    tools=[monitor_query_performance],
    llm=llm,
    system_prompt="You are a database performance expert. Help users understand the performance characteristics of their SQL queries and provide specific recommendations for optimization.",
    can_handoff_to=["DB Explorer", "Query Analyzer"]
)



    # Create the agent with the required fields
    explorer_agent = FunctionAgent(
        name="DB Explorer",
        ...
        can_handoff_to=["Query Analyzer", "Performance Monitor"],

    )
    
    analysis_agent = FunctionAgent(
        name="Query Analyzer",
        description="An agent that analyzes SQL queries for performance and provides recommendations",
        tools=[analyze_query_complexity],
        llm=llm,
        system_prompt="You are a SQL performance expert. Help users understand and optimize their database queries by analyzing their complexity and providing specific recommendations for improvement.",
        can_handoff_to=["DB Explorer", "Performance Monitor"],
    )
    # Initialize the workflow
    workflow = AgentWorkflow(
        agents=[orchestration_agent, explorer_agent, analysis_agent, performance_agent],
        root_agent=orchestration_agent.name,
        )
    
    # Get query from command line argument or use default
    #default_query = "Which tables exist in the database"
    default_query = "Get me all data related to the company Airline Leader Corp"
